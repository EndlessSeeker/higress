...
前三类后端始终使用 SSE；`SendMessage` 在网关有界聚合，因此也适用于不支持原生 blocking 的 Dify Agent 和异步 Coze Chat。Cloud Agent 创建 session 后先建立 SSE，再提交 user.message，避免 tail-only SSE 漏掉快速输出。CMA 使用 Managed Agents API，**不是** Claude Messages API。
...
| `apiBasePath` | Cloud 路径前缀；Qoder 默认 `/api/v1/cloud`，CMA 默认 `/v1` |
...
Dify `message_end`、百炼 `finish_reason:stop`、Coze `conversation.chat.completed`、Cloud 根 session 的 `session.status_idle` + `stop_reason.type:end_turn` 才表示完成。子线程 idle 不结束任务。外部操作需求映射 `TASK_STATE_INPUT_REQUIRED`；异常结束、非法事件、超限、没有终态的 EOF 映射 `TASK_STATE_FAILED`。Cloud 可恢复 `session.error` 只作为进度暴露，等待后续状态。
